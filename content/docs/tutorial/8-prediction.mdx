...
It extends the contract by storing trigger metadata, validates signed AVS outputs, and also interacts with the external contracts mentioned above (`PredictionMarketFactory`, `MarketMaker`, and `ConditionalTokens`).
...
Take a look at the full code to get an idea of how the contract is structured: https://github.com/Lay3rLabs/wavs-demos/blob/main/src/PredictionMarketOracleController.sol
...
You can run the prediction market demo locally by following the steps from the [README](https://github.com/Lay3rLabs/wavs-demos/blob/noah/prediction-market/PREDICTION_MARKET_DEMO.md).
