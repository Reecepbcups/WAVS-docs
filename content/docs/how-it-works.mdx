...
WAVS is a decentralized execution framework for AVSs (Actively Validated Services), enabling verifiable offchain computation triggered by onchain events. It provides a runtime for executing WASM-based service components, allowing developers to define event-driven offchain workflows while inheriting Ethereum’s security via EigenLayer restaking.

When an onchain contract emits an event, WAVS operators listen, execute the corresponding service component offchain, and submit the signed result back onchain for verification. This allows AVSs to process complex computations offchain at near-native speed, while maintaining onchain verifiability and trust-minimized execution.

The WAVS platform is made up of of service components, operator networks, and onchain contracts, forming a decentralized execution layer that extends the capabilities of smart contracts to offchain executions while maintaining verifiability and decentralization.


## The flow

Before diving into each part individually, it's important to understand the basic execution flow of an Service using WAVS.
...
1. An AVS builder defines their service with a trigger, a service component, and submission.
...
The WAVS platform consists of several different parts which work together to form a decentralized offchain execution layer for AVSs whose results can be verified onchain.
...
- [The WAVS runtime](#wavs-runtime): the WASI environment that powers WAVS' offchain computation.
- [Operators](#operators): node participants that opt into running services.


## Triggers

When creating a service, an AVS builder will specify which onchain events will trigger a service component to be run by operators. Triggers are onchain events that are used to prompt a task to be run. A trigger contract can be
...
The Service Manager acts as the intermediary between EigenLayer and WAVS, ensuring that execution results are properly verified before being processed further. It validates data and operator signatures, enforces security policies such as EigenLayer restaking, and forwards verified results to the appropriate Service Handler for final processing.

### Service Handler Contract

The Service Handler is responsible for receiving and processing validated data from the Service Manager. It provides an interface for handling service execution, allowing developers to define their own custom logic for storing, interpreting, or further processing results. Since the Service Handler is customizable, AVS developers can implement different rules depending on the specific needs of their service.
